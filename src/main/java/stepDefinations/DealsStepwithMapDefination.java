...
//	@When("^user enters username as \"([^\"]*)\" and password as \"([^\"]*)\"$")
	...
	//	@When("^user enters username as \"([^\"]*)\" and password as \"([^\"]*)\"$")
	public void user_enters_username_as_and_password_as(DataTable credentials) throws InterruptedException {
	...
	public void user_enters_Title_as_and_Stage_as_and_Status_as(DataTable dealDetails) throws InterruptedException {
		...
		{	
			Deals.txtTitleDeals(driver).clear();
			Deals.txtTitleDeals(driver).sendKeys(data.get("title"));
			
			Deals.cmbStageDeals(driver).sendKeys(data.get("stage"));
			Thread.sleep(2000);
			Deals.cmbStageDeals(driver).sendKeys(Keys.ENTER);
			
			Deals.cmbStatusDeals(driver).sendKeys(data.get("status"));
			Thread.sleep(2000);
			Deals.cmbStatusDeals(driver).sendKeys(Keys.ENTER);
...
//		Deals.btnSave(driver).click();
		System.out.println("Save button clicked in prev step");
	}
